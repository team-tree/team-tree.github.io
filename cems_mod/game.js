...
PS.touch = function( x, y, data, options ) {
	"use strict"; // Do not remove this directive!	
	var colors = [PS.COLOR_RED, PS.COLOR_ORANGE, PS.COLOR_YELLOW, PS.COLOR_GREEN, PS.COLOR_BLUE, PS.COLOR_VIOLET];
	var six_letter = ["PIZZAZ", "FRIZZY", "HIJACK", "JINXED", "BUZZED"];
	var five_letter	= ["PIZZA", "JUMPY", "QUACK", "JUMBO", "WOOZY"];
	var four_letter = ["COOL", "SASS", "BAND", "CASH", "JOKE"];
	var three_letter = ["AND", "FAN", "TIN", "CAN", "HIM"];
	var two_letter = ["YO", "ME", "HI", "PI", "HA"];
	var one_letter = ["A", "D", "G", "S", "R"];
	var current_word;
	...
	switch(x){
		case 0:
			current_word = six_letter[y];
			break;
		case 1:
			current_word = five_letter[y];
			break;
		case 2:
			current_word = four_letter[y];
			break;
		case 3:
			current_word = three_letter[y];
			break;
		case 4:
			current_word = two_letter[y];
			break;
		case 5:
			current_word = one_letter[y];
			break;
	...
	for(var i = 0; i < current_word.length; i++){
		...
		PS.data(x, y, current_word.charAt(i));
		PS.color(x, y, colors[rand]);
		x++;

	}

	//PS.debug("PS.touch(): x = " + x + ", y = " + y + "word =" + current_word + "\n");
	...
	// Checking how long the word displayed should be
